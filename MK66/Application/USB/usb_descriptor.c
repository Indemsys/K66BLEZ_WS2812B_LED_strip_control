...
{
#ifdef _USB_DEBUG
  USB_PRINTF("USB_Desc_Get_Descriptor (%08X): ", handle);
  switch (type)
  {
  ...
  {
    // «десь перенастраиваем размеры буферов конечных точек на максимальный размер в соответствии с текущей скоростью интерфейса
    // ѕоскольку после событи€ USB_DEV_EVENT_BUS_RESET скорость могла быть определена не корректно, так как в последствии скорость интерфейса
    // измен€етс€ после обрабоки прерывани€ по флагу EHCI_STS_PORT_CHANGE
...
#ifdef _USB_DEBUG
  USB_PRINTF("USB_Desc_Set_Speed (%08X, %d)\r\n", handle, speed);
#endif
