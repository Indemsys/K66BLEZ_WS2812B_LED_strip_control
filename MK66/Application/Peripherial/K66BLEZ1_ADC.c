...
/*
  ...
  - Инициализируется ADC с флагом DMA и старотом от аппаратного тригера
  ...
  - Инициализируется FTM (генератор PWM) с генерацией внещнего сигнала тригероа.
      FTM конфигурируется с центрально симметричным PWM. На серединах импульсов PWM генерируется сигнал тригера запускабщий ADC
...
static void DMA_ADC_Isr(void)
{
  ...
  // Копируем результаты DMA в рабочую переменную adcs
...
//  adcs.smpl_Bandgap = adc1_results[7];
  ...
  // Inputs_set_ADC_evt(); // В прерывнии уровня ядра сервисы RTOS использовать не можем
}
...
static void ADC_start_DMA(uint8_t n, uint8_t enint)
{
  ...
  DMA->TCD[res_ch].DADDR = (uint32_t)&init_ADC_DMA_cbl[n].results_vals[1]; // Адрес приемника данных из ADC - массив отсчетов
  ...
  DMA->TCD[cfg_ch].SADDR = (uint32_t)&init_ADC_DMA_cbl[n].config_vals[2];     // Источник - массив настроек для ADC
...
void ADC_calibr_config_start(void)
{
  ...
  ADC0_SC1A = adc0_ch_cfg[0];
  ADC1_SC1A = BIT(6) + adc1_ch_cfg[0]; // Разрешаем прерывания от ADC1  
  Install_and_enable_kernel_isr(INT_ADC1, ADC_PRIO, ADC_Isr); // Инсталлируем прерывание ADC1
